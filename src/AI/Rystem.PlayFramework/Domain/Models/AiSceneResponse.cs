...
public sealed class AiSceneResponse
{
    ...
    /// <summary>
    /// Function/tool arguments as a JSON document (if applicable).
    /// Per-tool responses with <see cref="FunctionName"/> set contain at least an empty
    /// JSON object. Aggregate function-request responses leave this property unset.
    /// Arguments may contain sensitive application data and should not be logged blindly.
    /// </summary>
    ...
    /// <summary>
    /// Identity of the runtime description catalog actually acquired for this request.
    /// Null only for responses produced before catalog acquisition.
    /// </summary>
    public RuntimeDescriptionExecutionInfo? RuntimeDescriptions { get; set; }

    /// <summary>
    /// Multi-modal output contents (images, audio, files generated by LLM or tools).
    /// Internal property — use <see cref="SerializableContents"/> for JSON serialization.
    /// </summary>
    [JsonIgnore]
    public IEnumerable<AIContent>? Contents { get; set; }

    /// <summary>
    /// JSON-friendly contents mapped from <see cref="Contents"/>.
    /// Serializes as <see cref="ClientContentItem"/> array to match TypeScript client contract.
    /// </summary>
    [JsonPropertyName("contents")]
    public List<ClientContentItem>? SerializableContents
    {
        get => Contents?.Select(c => c switch
        {
            TextContent text => new ClientContentItem { Type = "text", Text = text.Text },
            DataContent data => new ClientContentItem
            {
                Type = "data",
                Data = data.Data is { Length: > 0 } ? Convert.ToBase64String(data.Data.ToArray()) : null,
                MediaType = data.MediaType
            },
            _ => new ClientContentItem { Type = "text", Text = c.ToString() }
        }).ToList();
        set => Contents = value?.Select(item => item.ToAIContent());
    }
